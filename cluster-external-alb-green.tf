module "green_cluster_external_alb" {
  ...
  target_groups_count = "${var.green_external_alb_target_groups_count}"
  target_groups       = "${var.green_external_alb_target_groups}"

  route53_aliases_name = ["${var.cluster_name}-external-alb-green"]
  route53_zone_id      = "${var.external_alb_route53_zone_id}"
}

resource "aws_route53_record" "green_cluster_external_alb" {
  ...
  records        = ["${module.green_cluster_external_alb.dns_name}"]
}
