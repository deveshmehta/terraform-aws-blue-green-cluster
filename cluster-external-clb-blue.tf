module "blue_cluster_external_clb" {
  ...
  route53_aliases_name = ["${var.cluster_name}-external-clb-blue"]
  route53_zone_id      = "${var.external_clb_route53_zone_id}"
}

resource "aws_route53_record" "blue_cluster_external_clb" {
  ...
  records        = ["${module.blue_cluster_external_clb.dns_name}"]
}
