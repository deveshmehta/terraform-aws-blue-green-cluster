module "green_cluster_internal_clb" {
  ...
  route53_aliases_name = ["${var.cluster_name}-internal-clb-green"]
  route53_zone_id      = "${var.internal_clb_route53_zone_id}"
}

resource "aws_route53_record" "green_cluster_internal_clb" {
  ...
  records        = ["${module.green_cluster_internal_clb.dns_name}"]
}
